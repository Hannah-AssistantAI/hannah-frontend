...
const ChallengesManagement: React.FC = () => {
  ...
  const handleSuggestionApproved = useCallback((data: { subjectId: number; suggestionId: number; contentType: string; content: string }) => {
    console.log('[Challenges] Suggestion approved event:', data);
    if (selectedCourse && data.subjectId === selectedCourse.id && data.contentType === 'CommonChallenge') {
      // Update local state: move from pending to approved style
      setChallenges(prev => prev.map(s =>
        s.id === data.suggestionId ? { ...s, status: SuggestionStatus.Approved } : s
      ));
      // Add to official challenges
      setSubjectChallenges(prev => [...prev, data.content]);
      toast.success('Your challenge was approved!');
    }
  }, [selectedCourse]);
  ...
  const handleSuggestionRejected = useCallback((data: { subjectId: number; suggestionId: number; contentType: string }) => {
    console.log('[Challenges] Suggestion rejected event:', data);
    if (selectedCourse && data.subjectId === selectedCourse.id && data.contentType === 'CommonChallenge') {
      // Update local state
      setChallenges(prev => prev.map(s =>
  ...
  const handleSuggestionDeleted = useCallback((data: { subjectId: number; suggestionId: number; contentType: string }) => {
    console.log('[Challenges] Suggestion deleted event:', data);
    if (selectedCourse && data.subjectId === selectedCourse.id && data.contentType === 'CommonChallenge') {
      // Remove from local state
      setChallenges(prev => prev.filter(s => s.id !== data.suggestionId));
    }
  }, [selectedCourse]);
