...
interface UseChatMessagesParams {
    ...
    selectedModel?: string;  // NEW: Model selection from UI
    onModelPreferenceLoaded?: (model: string) => void; // NEW: Callback when model preference is loaded
}
...
export const useChatMessages = ({
    ...
    selectedModel = 'auto',  // NEW: Default to auto mode
    onModelPreferenceLoaded // NEW: Callback
        ...
        const sendInitialQuery = async () => {
            ...
            try {
                ...
                const response = await chatService.sendTextMessage(
                    ...
                    initialQuery,
                    subjectId || undefined,
                    selectedModel  // Pass model selection
                );
        ...
        if (conversationId && !initialQuery && user?.userId) {
            const loadConversationHistory = async () => {
                try {
                    ...
                    if (conversationDetails.modelPreference && onModelPreferenceLoaded) {
                        console.log('🤖 Model Preference from loaded conversation:', conversationDetails.modelPreference);
                        onModelPreferenceLoaded(conversationDetails.modelPreference);
                    }
