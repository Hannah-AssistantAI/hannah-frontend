              ...
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-2">
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${r.status?.toLowerCase() === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>
                      <span className={`inline-block w-2 h-2 rounded-full ${r.status?.toLowerCase() === 'pending' ? 'bg-amber-500' : 'bg-green-500'}`}></span>
                      {r.status}
                    </span>
                  </td>
                  <td className="px-4 py-2">
